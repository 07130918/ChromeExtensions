<template>
    <div class="to-do-list">
        ...
        <div v-for="list in toDoList" :key="list.content.value">
            <input type="checkbox" :checked="list.isInputable" @change="updateIsInputable(list, $event)">
            <input type="text" :value="list.content" @change="updateContent(list, $event)" autocomplete="off" :disabled="list.isInputable">
        </div>
        <button class="btn btn-danger" @click="toDoList=createList(listLength, 'toDoList')">Reset</button>
    </div>
</template>
